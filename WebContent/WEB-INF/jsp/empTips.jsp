...
$(function() {
	//消息提示框
 	...
 	var item = {  
	        ...
	        function gggT() {
	    		var atisname = document.getElementById("atisName");
	    		if(atisname.value==""){
	    			alert("贴士主题不能为空");
	    			return false;
	    		}else{
		...
		<div class="tCright">
			...
			<div class="motai"id = "div2">
				 <div style="height: 300px">
							...
							<form action="${pageContext.request.contextPath }/Tis/updateTis.emp" method="post" onsubmit="return gggT()">
						     	<div class="bt"><button class="btn btn-default" type="submit">发送</button></div>
								<div>
									<!-- 主题id -->
									<input name="id" value="${tis.id }" type="hidden" style="width:95%">
									 <label class="form-label" for="nameRoEmail">贴士名称:</label>
		                            <input class="form-control " type="text"  id="atisName" name="tisName" value="${tis.tisName }" placeholder="贴士名称" >
		                            <label class="form-label" for="nameRoEmail">贴士内容:</label>
						     		<textarea id="container" name="tisContent">${tis.tisContent }</textarea>
								</div>
						 	</form>
						</div> 
			</div>
