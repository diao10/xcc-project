...
function f1(id) {
	...
	$.ajax({
		...
		success:function(data){
			$.ajax({
				data:{id:id},
				url:"${pageContext.request.contextPath }/message/read.do"
			})
			...
			<div class="boxg1">
				...
				<div class="right">
					...
					<c:if test="${a.flag==0 }">
							...
							<a  onclick="f1(${a.id})"> <c:if test="${a.flag==0 }"><span class="circle" style="float: left"></span></c:if>
							
							<c:if test="${fn:length(a.mess_theme)>15}">
													<c:out value="${fn:substring(a.mess_theme, 0, 15)}..."></c:out>
												</c:if>
												<c:if test="${fn:length(a.mess_theme)<=15}">  
                         								${a.mess_theme}  
                   								</c:if></a>
						...
						<div class="page">
							<ul>
								  ...
								  <li  <c:if test="${page==1}">disabled</c:if>><a <c:if test="${page!=1}"> href="${pageContext.request.contextPath}/message/selMessage.do?page=${page-1}" </c:if>>&laquo; </a></li>
						   		 		<c:if test="${begin!=1 }"><li><a>...</a></li></c:if>
						   		 		<c:forEach var="i" begin="${begin}" end="${end}">
						   					<li <c:if test="${i==page }">class="active"</c:if>><a href="${pageContext.request.contextPath}/message/selMessage.do?page=${i}" >${i}</a></li>
						   				</c:forEach>
						   				<c:if test="${end!=pages }"><li><a>...</a></li></c:if>
						   			 <li <c:if test="${page==pages}">disabled</c:if>><a <c:if test="${page!=pages}"> href="${pageContext.request.contextPath}/message/selMessage.do?page=${page+1}" </c:if>> &raquo;</a></li>
							</ul>
						</div>
					</div>
					<div class="body" id = "div2" style="display: none">
							...
							<div  class="foot1">
								<a href='${pageContext.request.contextPath }/message/selMessage.do'>返回</a>
							</div>
					</div>
